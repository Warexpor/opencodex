...
翻譯路由一律會把系統指令末尾的 Claude 工具提示（`<total_tokens>N tokens left</total_tokens>`，或已知的 TaskCreate 提醒）移到最後一則使用者訊息。這樣指令前綴，以及沒有中繼資料的 Desktop `prompt_cache_key`，才不會跟著頁尾變動。不需要設定 `claudeCode.stabilizePromptCache`。程式碼圍欄中的範例和不符合的文字會保留，Anthropic 原生轉送不變。這不會產生對話識別碼，也不保證上游快取命中。
