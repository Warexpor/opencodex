...
| `proxy?` | `string` | — | 對外 HTTP(S) 或 SOCKS5 代理 URL（`socks5://host:port`，`ocx start --socks5`）或 `${ENV_VAR}`。HTTP URL 僅在那些變數未設定時套用至 `HTTP_PROXY` / `HTTPS_PROXY`；SOCKS5 寫入 `ALL_PROXY` 並使用 OpenCodex SOCKS5 傳輸。回送保留在 `NO_PROXY` 中。 |
