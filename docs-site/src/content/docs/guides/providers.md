...
(`User-Agent: opencode/1.18.31`, `x-opencode-client: cli`, plus per-key `x-opencode-session` /
per-call `x-opencode-request`) so free models unlock without an external gateway.
Session and request ids use OpenCode's timestamp form (`ses_` / `msg_` plus 12 hex and 14
base62). A saved unversioned `User-Agent: opencode`, a `desktop` client marker, and a
session id that is not that form are upgraded at request settle. An explicit
`opencode/<semver >= 1.18.0>` user agent is left alone. Free-tier chat and responses
bodies also carry OpenCode's builtin tools and `stream: true`; a non-streaming Chat
client still receives one JSON completion.
...
If Zen still answers `MissingSessionID` after that fingerprint, the client error says
the identity was sent and points at the keyed `opencode-zen` provider
([opencode.ai/auth](https://opencode.ai/auth)). Upstream terms:
