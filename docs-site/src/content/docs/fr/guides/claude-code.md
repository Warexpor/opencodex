...
Les routes traduites déplacent toujours une notice Claude en fin d'instructions système (`<total_tokens>N tokens left</total_tokens>`, ou le rappel TaskCreate connu) vers un dernier message utilisateur. C'est ce qui garde le préfixe des instructions, et le `prompt_cache_key` Desktop sans métadonnées, stables d'un tour à l'autre. `claudeCode.stabilizePromptCache` n'est pas requis. Les exemples dans des blocs de code et le texte non reconnu sont conservés ; le transfert Anthropic natif reste inchangé. Cette option ne crée pas une identité de conversation et ne garantit aucun succès du cache amont.
