...
Çevrilen rotalar, sistem talimatlarının sonundaki Claude bildirimini (`<total_tokens>N tokens left</total_tokens>` veya bilinen TaskCreate hatırlatması) her zaman son kullanıcı mesajına taşır. Böylece talimat öneki ve meta verisiz Desktop `prompt_cache_key` turlar arasında sabit kalır. `claudeCode.stabilizePromptCache` gerekli değildir. Kod bloklarındaki örnekler ve eşleşmeyen metin korunur; yerel Anthropic aktarımı değişmez. Bu seçenek konuşma kimliği oluşturmaz ve üst hizmette önbellek isabeti garanti etmez.
