...
На маршрутах с преобразованием концевое уведомление Claude (`<total_tokens>N tokens left</total_tokens>` или известное напоминание TaskCreate) всегда переносится из системных инструкций в последнее пользовательское сообщение. Так префикс инструкций и Desktop `prompt_cache_key` без метаданных не меняются от хода к ходу. `claudeCode.stabilizePromptCache` не требуется. Примеры в блоках кода и нераспознанный текст сохраняются; нативная передача Anthropic не меняется. Идентификатор разговора не создаётся, попадания в кэш не гарантируются.
