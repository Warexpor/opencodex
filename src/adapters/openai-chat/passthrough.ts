...
export function buildOpenAIChatPassthroughRequest(
  ...
  const outbound = isOpenCodeZenProvider(provider)
    ? applyOpenCodeZenFreeTierBody(body, "chat", modelId)
    : body;
  const bodyJson = JSON.stringify(outbound);
  ...
  if (isDebugEnabled()) {
    ...
    debugProviderDiagnostic("openai-chat", "passthrough-request", {
      ...
      stream: outbound.stream === true,
      messageCount: Array.isArray(outbound.messages) ? outbound.messages.length : 0,
      toolCount: Array.isArray(outbound.tools) ? outbound.tools.length : 0,
      hasCredential,
