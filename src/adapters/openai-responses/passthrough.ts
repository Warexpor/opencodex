...
import { applyOpenCodeZenFreeTierBody, isOpenCodeZenProvider } from "../../providers/opencode-zen-transport";
...
import { applyDefaultResponsesReasoningEffort, mapRoutedResponsesReasoningEffort, normalizeConfiguredReasoningSummaryDelivery, sanitizeReasoningInputContent, stripDisabledReasoningSummaries, stripDisabledVerbosity, stripUnsupportedReasoningSummaryDelivery } from "./reasoning";
...
export function createResponsesPassthroughAdapter(provider: OcxProviderConfig): ProviderAdapter & { passthrough: true } {
  return {
    ...
    buildRequest(parsed: OcxParsedRequest, incoming: IncomingMeta) {
      ...
      const wireBody = isPlainObject(finalBody) && isOpenCodeZenProvider(provider)
        ? applyOpenCodeZenFreeTierBody(finalBody, "responses", parsed.modelId)
        : finalBody;
