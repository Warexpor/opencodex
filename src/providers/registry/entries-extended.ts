...
export const PROVIDER_REGISTRY_EXTENDED: readonly ProviderRegistryEntry[] = [
  ...
  },
  {
    ...
    note: "Keyed OpenCode Zen gateway with an in-tree free-tier CLI identity path (User-Agent opencode/1.18.31, x-opencode-client=cli, Identifier.create ses_/msg_ headers). Free models on this tier are often short-window rate-limited at roughly 15-20 requests/minute (community-measured; OpenCode does not publish RPM). Zen may return generic 429s without Retry-After / X-RateLimit headers; when Retry-After is omitted, opencodex adds a synthetic backoff hint (upstream Retry-After still wins). Distinct from the keyless opencode-free quota (~200 Big Pickle/free-model requests per 5 hours). Muse Spark ids ride /v1/responses. Docs: https://opencode.ai/docs/zen/. Free-model prompts may be retained for training — do not send confidential material. Outbound SOCKS5: ocx start --socks5 (default 127.0.0.1:10808).",
    // Console rejects free models unless the caller looks like OpenCode >= 1.18.0.
    // Session/request ids and the body shape are stamped at settle time.
    staticHeaders: {
  ...
  },
  {
    ...
    featured: true,
    liveModels: true,
    note: "No key needed — public free tier with in-tree OpenCode CLI identity (User-Agent opencode/1.18.31, x-opencode-client=cli, Identifier.create x-opencode-session). OpenCode currently advertises about 200 Big Pickle/free-model requests per 5 hours. The same Zen gateway can also short-window rate-limit free models at roughly 15-20 requests/minute, and may return generic 429s without Retry-After (opencodex synthesizes backoff only when that header is omitted). If Zen still answers MissingSessionID (\"OpenCode's free tier can only be used in OpenCode\"), the fingerprint was rejected; the keyed opencode-zen provider is the supported alternative (https://opencode.ai/auth). Free models are discovered live from Zen. Muse Spark free ids use /v1/responses with default reasoning.effort=minimal. Data use: per OpenCode's Zen docs (https://opencode.ai/docs/zen/), prompts sent to free models may be retained and used for training/improvement — do not send confidential material through this provider. Outbound SOCKS5: ocx start --socks5.",
    dashboardUrl: "https://opencode.ai",
    staticHeaders: {
      // Console's free-tier gate (anomalyco/opencode#49433) requires OpenCode >= 1.18.0.
      // A bare "opencode" or runtime User-Agent is rejected. Operators can still override
      // either value through the provider headers API.
      "User-Agent": "opencode/1.18.31",
      "x-opencode-client": "cli",
    },
    modelWireDefaults: Object.fromEntries(OPENCODE_ZEN_MUSE_MODELS.map(id => [id, "openai-responses" as const])),
