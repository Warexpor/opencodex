 ...
 *
 * Presence of a valid Identifier.create session is part of Console's gate. This fork
 * stamps that header, a versioned User-Agent, and OpenCode's builtin tools. If Zen
 * still refuses, the user-facing error says the identity was sent and names the
 * keyed route, instead of leaking a raw MissingSessionID.
 */
...
const FREE_TIER_ENRICHMENT_MARKER = "already sent an OpenCode-shaped session header";
...
export function enrichOpenCodeZenFreeTierMessage(
  ...
  return (
    `${message}`
    + " OpenCode Zen's keyless free tier admits only OpenCode's own client and checks x-opencode-session."
    + ` opencodex ${FREE_TIER_ENRICHMENT_MARKER} and a versioned User-Agent.`
    + " Zen still refused this request."
