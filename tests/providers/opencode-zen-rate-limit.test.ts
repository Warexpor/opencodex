...
describe("opencode-free keyless tier lock-in (#4121)", () => {
  ...
  test("registry note states the fingerprint, the remaining refusal, and the keyed alternative", () => {
  ...
  test("the client error explains the gate and names the supported keyed route", () => {
    ...
    expect(enriched).toContain("already sent an OpenCode-shaped session header");
  });
